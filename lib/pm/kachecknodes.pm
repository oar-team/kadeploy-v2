...
my @service_list=("ICMP","RSH","SSH","MCAT");
my $sleeptime=5;
...
sub check_result()
{
	...
	if (@type_list)
	{
	    ...
	    {
		my $printable=$node->get_name()." ".$service." ".$db->get_nodestate($node->get_name(),$service);
		if ($printable =~ /DOWN/)
		{ $message->message(1,$printable); }
		else 
		{ $message->message(-1,$printable); }
	    }
	}
	...
	if (! ( $db->get_nodestate($node->get_name(),"MCAT") eq "UP")) { $ok=0; }
    }
    $db->disconnect();
    return $ok;
}
