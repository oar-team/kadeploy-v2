...
\item \texttt{kernel}: path of the kernel in the tarball (must not be a link).
\item \texttt{kernel\_params}: set of parameters that must be applied to the kernel for a correct boot.
\item \texttt{initrd}: path of the initrd in the tarball (must not be a link).
\item \texttt{hypervisor (opt)}: path of the hypervisor in the tarball (must not be a link). This fields is only required for the Xen based environments.
