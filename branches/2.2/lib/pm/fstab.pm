...
sub get()
{
	...
	{ $ret=$self->get_linux(); }
	if ($ostype && $ostype eq "solaris")
	{ $ret=$self->get_solaris(); }
...
sub get_linux()
{
    ...
    for ( my $i=1 ; $i< $disk->get_numberofpartition()+1; $i++)
    {
	$linuxdevice=libkadeploy2::device::new();
	$linuxdevice->set_type($diskinterface);
	$linuxdevice->set_disknumber($disknumber);
	$linuxdevice->set_partnumber($i);
