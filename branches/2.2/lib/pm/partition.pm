...
sub set_ostype($)
{
    ...
    my $ostype=shift;
    my $ok=0;
    if ($ostype =~ /linux|windows|solaris|netbsd|freebsd|openbsd|dos/i) 
    { 
	$ok=1; 
...
sub check()
{
    ...
    elsif ($self->get_type() eq "slice")
    {
	if ($self->get_ostype() eq "")
	{
	    $message->message_checkfailed("type=slice && ostype=??");
	}
    }
    else
...
sub addtodb($$)
{
	...
	else          { $message->message(2,"Fail to register partition ".$self->get_number." for disk $disknumber of node $nodename"); }
