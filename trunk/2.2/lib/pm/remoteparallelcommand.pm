...
sub check()
{
    ...
    { print "INTERNAL ERROR : parallellauncher must be : dsh | DKsentinelle | internal\n"; exit 1; }
    if ( !($self->{connector} eq "rsh" ) &&
	 !($self->{connector} eq "ssh" ) )
    { print "INTERNAL ERROR : connector must be : rsh | ssh\n"; exit 1; }
    if (!($self->{timeout}>=0)) { print "INTERNAL ERROR timeout :".$self->{timeout}."(must be >= 0)\n"; exit 1; }
}
...
sub exec_internal()
{
	...
	if ($exit_value!=0 ||
	    ...
	    $dumped_core!=0)
	{
	    print "ERROR: [".$pid2node{$pid}.":".$pid2cmd{$pid}."] exit_value:$exit_value signal_num:$signal_num dumped_core:$dumped_core\n";
	    $ok=0;
	}
    }
    return $ok;
