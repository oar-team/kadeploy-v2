...
sub run()
{
	...
	if ($kernel)
	{
	    ...
	    {
		$pxelinuxtftp->set_partnumber($partnumbercmdline);
		if ($disknumbercmdline)
		{ $pxelinuxtftp->set_disknumber($disknumbercmdline); 		}
		else
		{ $pxelinuxtftp->set_disknumber(1); 		}
	    }
	    else
	    {
		$pxelinuxtftp->set_initrdroot();
	    }
	    $pxelinuxtftp->set_linux();
	    $pxelinuxtftp->writepxelinuxcfg();
	}
    }
    elsif ($type eq "nfsrootlinux")
    {
	$pxelinuxtftp=libkadeploy2::pxelinuxtftp::new(); 
	$pxelinuxtftp->set_nodelist($nodelist);
	...
	if ($kernel)
	{
	    ...
	    $pxelinuxtftp->set_initrd($initrd);
	    $pxelinuxtftp->set_kernelparams($kernelparams);
	    $pxelinuxtftp->set_nfsroot();
	    $pxelinuxtftp->set_linux();
	    $pxelinuxtftp->writepxelinuxcfg();
	}

    }
    elsif ($type eq "floppy")
    {
	$pxelinuxtftp=libkadeploy2::pxelinuxtftp::new(); 
	$pxelinuxtftp->set_nodelist($nodelist);	
    }
    ...
    {
	if (
	    ($type eq "pxelinux" || $type eq "nfsrootlinux") &&
	    $kernel
	    ) 
	{	
	...
	$errorhandler->error_in("run","Something wrong occured... you're here ?!");
    }
...
sub check_options()
{
    ...
    else { $message->missing_cmdline(2,"nodes"); $ok=0; }

    if ((! $type) && (!$kernel) && $nodelist)
    {	
	if (display_pxetype($nodelist))
	{ $errorhandler->program_exit(); }
	else
	{ $errorhandler->error_in("display_pxetype");}

    }
    
    if ($type eq "floppy" && (! $floppypath))
    {  	$errorhandler->error_in("check_options","Missing floppy params");      }

    if ($type eq "pxegrubfloppy")   { $fromdisk=1; }
    if ($fromdisk && $fromtftp)     
    { $errorhandler->error_in("checkoption","fromdisk and fromtftp excluded...");  }

    if ($fromdisk)        { $fromtftp=0;  }
    if ($fromtftp)        { $fromdisk=0;  }

    if ($fromdisk && (! $disknumbercmdline))
    { $disknumbercmdline=1; }
    ...
    if (! ($type =~ /pxelinux|pxegrub|pxeopenbsd|pxegrubfloppy|pxewindowsfloppy|floppy|nfsrootlinux/))
    {
	$errorhandler->error_in("checkoption","type != pxelinux|pxegrub|pxeopenbsd|pxegrubfloppy|pxewindowsfloppy|floppy|nfsrootlinux");
    }

    if ( (! ($type =~ /^pxe.+|floppy|nfsrootlinux/)) && (! $partnumber))
    { $errorhandler->error_in("checkoption","partnumber needed");   }   

    if ($partnumbercmdline)
    {
	$disks=libkadeploy2::disks::new($nodelist);

	if (! $disks->check_disk_type($disknumber)) 
	{ $errorhandler->error_in("checkoption","check_disk_type($disknumber) failed"); }
...
sub setup_pxebootloader($$)
{
    ...
    elsif ($pxebootloader eq "floppy")
    {
	$pxeloaderpath=$pxelinuxpath;
	$bootloadername="'pxelinux (pxelinux-memdisk:$floppypath'";
    }
    ...
    elsif ($pxebootloader eq "nfsrootlinux")
    {
	$pxeloaderpath=$pxelinuxpath;
	$bootloadername="'pxelinux (pxelinux-linux-nfsroot)'";
    }
    else
    {
	die "internal error";
...
sub copy_bootfile_tftpnodes($$)
{
	...
	if ($i==0)
	{
	    ...
	    $node_name_src=$node_name;
	}
	else { link_tftpnode($bootfile,$node_name_src,$node_name); }
...
{
    my $source=shift;
    my $node_name_src=shift;
    my $node_name_dst=shift;
    my $ok=1;
    if ( -r "$source")
    { 
	my @split=split("/",$source);
	my $suffix=$split[$#split];
	$message->message(-1,"linking $suffix | $node_name_src/$source | to $tftproot/$node_name_dst/$source");
	link("$tftproot/$node_name_src/$suffix","$tftproot/$node_name_dst/$suffix");
    }
    else
    { 
	$message->message(2,"Can't link $source to $tftproot/$node_name_dst"); 
	$ok=0; 
    }       
    return $ok;
}
