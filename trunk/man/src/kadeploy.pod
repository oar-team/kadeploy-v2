...
B<kadeploy>  B<-e|--environment> environment_name [B<-l> username] B<-p|--partition> partition [B<-m|--machine> hostname] [B<-f> nodefile] [B<-v|--verbose>]
...
=item kadeploy -e debian4all -p sda3 -m node7

Deploys the debian4all environment on partition sda3 of node7.

=item kadeploy -e debian4jul -l jleduc -p sda3 -f $OAR_NODEFILE

Deploys the debian4jul environment user jleduc on partition sda3 of a set of nodes reserved with OAR in the deploy queue.

=back

=head1 SEE ALSO

kaenvironments(1), karecordenv(1)
