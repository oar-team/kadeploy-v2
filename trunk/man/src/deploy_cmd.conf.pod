...
The syntax is I<host_name> I<command_type> I<command> where possible command types are B<softboot>, B<hardboot>, B<deployboot>, B<console> or B<configuration>.
...
=head1 CONFIGURATION COMMAND

The configuration command is a directive to define the B<deploy.conf> type configuration file that should be used for the node. It is an optional directive: by default B<deploy.conf> file will be used. The configuration file specified should be named B<deploy-CLUSTERNAME.conf>, reside in the B</etc/kadeploy/> directory and should follow the syntax rules defined in the B<deploy.conf> file. For more information, please refer to the corresponding man page.
For example:
 node.cluster.domain configuration /etc/kadeploy/kadeploy-cluster.conf
