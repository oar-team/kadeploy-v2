...
B<kareboot>  [[B<-n|--noreboot>] | [B<-s|--soft>] | [B<-h|--hard>] | [B<-d|--deploy>]] [[B<-e|--environment> environment_name] | [B<-p|--partition> partition]] [B<-m|--machine> hostname] [B<-f> nodefile] [B<-v|--verbose>]
